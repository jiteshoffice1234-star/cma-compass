...
export function QuizOverlay({ chapterId, onClose }: { chapterId: number; onClose: () => void }) {
  ...
  const { current, idx, selected, answered, finished, total, choose, next, getResults, restart } =
    useQuizSession(chapterQuestions, chapterQuestions.length)
  ...
  useEffect(() => {
    if (finished) {
      const { score, perfect, attempts } = getResults()
      completeQuiz(chapterId, score, total, perfect, attempts)
  ...
  const { score, passed, perfect } = finished ? getResults() : { score: 0, passed: false, perfect: false }
  ...
  return (
    <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 10px' }}>
        ...
        {!finished && <div className="mono" style={{ fontSize: 15, fontWeight: 800, fontFamily: font.mono }}>{idx + 1} / {total}</div>}
        <div style={{ width: 36 }} />
      </div>
      {!finished && (
        <div style={{ padding: '0 16px 12px' }}>
          <div style={{ height: 12, background: color.card, border: border.thin, borderRadius: 6, overflow: 'hidden' }}>
            <motion.div animate={{ width: `${((idx) / total) * 100}%` }} style={{ height: '100%', background: color.secondary }} />
          </div>
        </div>
      )}
              ...
              {answered && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} style={{ marginTop: 20 }}>
                  <Button variant="secondary" onClick={next} style={{ width: '100%' }}>
                    {idx + 1 < total ? 'Next' : 'See Results'} <ArrowRight size={18} />
                  </Button>
                </motion.div>
              )}
            </AnimatePresence>
        ...
        {finished && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 260, damping: 18 }}
              ...
              <div className="mono" style={{ fontSize: 32, fontWeight: 800, color: passed ? color.success : color.danger, fontFamily: font.mono }}>{score}/{total}</div>
            </motion.div>
            ...
            <div style={{ color: color.muted, fontSize: 14, marginTop: 6, fontWeight: 600 }}>
              {passed ? `+${20 + (perfect ? 15 : 0)} XP earned` : `You need ${Math.ceil(total * 0.6)}/${total} to pass and unlock the next chapter.`}
            </div>
